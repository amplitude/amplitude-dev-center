...
|-|-|-|-|
`defaultTracking.sessions` | Optional. `boolean` | `true` | Enables session tracking. This configuration replaces [`trackingSessionEvents`](#configuration). If value is `true`, Amplitude tracks session start and session end events otherwise session events are not tracked. Note that when `false`, session Id is still tracked.<br /><br />See [Tracking sessions](#tracking-sessions) for more information.|
